...
def snapshot_initial_data():
    ...
    cur.execute("INSERT INTO transactions_snapshot SELECT * FROM transactions")
    cur.execute("INSERT INTO receivables_snapshot SELECT * FROM receivables")
    # Explicit column list (not SELECT *): checkout_abandonments has a
    # recovered_amount column (Phase 1 fix) that checkout_abandonments_snapshot
    # deliberately does not — the snapshot is taken before anything has been
    # executed, so recovered_amount is always NULL at snapshot time anyway,
    # and a bare SELECT * would break on column-count mismatch.
    cur.execute(
