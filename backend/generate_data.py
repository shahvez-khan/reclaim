...
Produces:
  ...
  - 50 overdue B2B receivables, days_overdue spread 7-90
...
def populate():
    ...
    cur.executemany(
        """INSERT INTO transactions
           (transaction_id, customer_id, amount, currency, payment_method,
            ...
            customer_opt_out, last_action_timestamp)
           VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
        txns,
    )
    ...
    total_txn = cur.execute("SELECT COUNT(*), SUM(amount) FROM transactions").fetchone()
    ...
    for row in cur.execute(
        """SELECT failure_code, COUNT(*), SUM(amount)
           FROM transactions GROUP BY failure_code ORDER BY COUNT(*) DESC"""
    ):
    ...
    total_recv = cur.execute("SELECT COUNT(*), SUM(amount) FROM receivables").fetchone()
    ...
    for label, lo, hi in [("7-15 days", 7, 15), ("15-45 days", 15, 45), ("45-90 days", 45, 91)]:
        row = cur.execute(
            "SELECT COUNT(*), SUM(amount) FROM receivables WHERE days_overdue >= ? AND days_overdue < ?",
            (lo, hi),
        ).fetchone()
        cnt = row[0] or 0
        amt = row[1] or 0
    ...
    total_aband = cur.execute("SELECT COUNT(*), SUM(cart_value) FROM checkout_abandonments").fetchone()
    ...
    for label, lo_min, hi_min in [("warm (<1hr)", 0, 60), ("cooling (1-24hr)", 60, 24 * 60), ("cold (>24hr)", 24 * 60, 10**9)]:
        cnt = 0
        amt = 0.0
        for row in cur.execute("SELECT cart_value, abandoned_at FROM checkout_abandonments"):
